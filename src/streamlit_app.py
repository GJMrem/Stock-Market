...
#     'dropout': 0.2
# }
# model = models.RNNModel(1, **model_params).to(device)
# model.load_state_dict(torch.load('models/RNNModel.pt', weights_only=True))

client = mlflow.MlflowClient()
model_versions = client.search_model_versions(f"name='{model_name}' and tag.status='production'")
...
model_predictions = pd.DataFrame(model_predictions, columns=[quote_type],
                                 index=pd.date_range(start=date_to,
                                                     end=date_to+timedelta(days=horizon-1),
                                                     freq='B', name='Date'),
)
# model_predictions.loc[date_to.strftime('%Y-%m-%d')] = original_quotes.iloc[-1][quote_type]
...
# data = pd.merge(original_quotes[[quote_type]], model_predictions, on='Date', suffixes=('_original', '_predicted'))
